...
function initMobileNav(){
  const toggle=$('navToggle'),sidebar=document.querySelector('.sidebar'),nav=$('mainNav'),backdrop=$('navBackdrop');
  if(!toggle||!sidebar||!nav)return;
  const setOpen=open=>{sidebar.classList.toggle('nav-open',open);toggle.setAttribute('aria-expanded',open?'true':'false')};
...
function initPanelManager(){
  ...
  style.textContent+=' html,body{max-width:100%;overflow-x:hidden}.app{padding-right:256px}.main{margin:0 auto}.sidebar{position:fixed;right:0;top:0;bottom:0;height:100vh;overflow:hidden}.app,.main,.grid,.panel,.results-panel{min-width:0;max-width:100%}.grid{display:flex;flex-direction:column}.pipeline{order:-1}.results-panel{width:100%}.panel-manager li span{white-space:normal;overflow:visible;text-overflow:clip;overflow-wrap:anywhere;word-break:break-word}.panel-manager.panels-page{max-width:100%;min-height:520px;margin-top:0}@media(max-width:1100px){.app{padding-right:78px}.main{min-width:0}.panel-manager.panels-page{min-width:0}}@media(max-width:680px){.app{padding-right:0;padding-top:62px}.sidebar{position:fixed;right:0;left:0;top:0;bottom:auto;width:100%;height:62px;z-index:60;overflow:visible;display:flex;align-items:center;justify-content:space-between;padding:0 14px}.sidebar .brand{padding:0;flex:1;min-width:0}.nav-toggle{display:flex}.sidebar nav{display:none;position:absolute;top:62px;right:0;left:0;flex-direction:column;overflow-y:auto;max-height:calc(100vh - 62px);background:#0d1424;border-bottom:1px solid #2a3854;box-shadow:0 20px 40px #00000066;padding:8px;gap:2px}.sidebar.nav-open nav{display:flex}.sidebar nav a{flex:0 0 auto;padding:13px 14px;border-radius:10px}.sidebar.nav-open~.nav-backdrop{display:block}.panel-manager{margin-bottom:14px;border-radius:12px}.panel-manager.panels-page{min-height:calc(100vh - 112px)}.panel-manager-head{padding:13px 14px}.panel-manager li{padding:10px 14px;gap:8px}.panel-manager li span{max-width:calc(100vw - 115px)}#addPanelForm{padding:11px 14px}.panel-manager ul{max-height:45vh}.table-wrap{max-width:100%;overflow-x:auto}.actions{max-width:100%;display:grid;grid-template-columns:repeat(2,1fr);gap:8px}.actions>:last-child{grid-column:1/-1}.button{flex:none;min-width:0;justify-content:center;text-align:center;font-size:12px;padding:11px 6px;line-height:1.3;gap:5px}}';
...
document.addEventListener('DOMContentLoaded',()=>{initSidebar();initMobileNav();initPanelManager();initLinksManager();initDbViewer();initAutoRunsManager();initLanguageToggle();initViews()});

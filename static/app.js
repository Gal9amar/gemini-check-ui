...
function api(url,method='GET',body=null){return new Promise((resolve,reject)=>{const request=new XMLHttpRequest();request.open(method,url,true);request.setRequestHeader('Accept','application/json');if(body)request.setRequestHeader('Content-Type','application/json');request.onload=()=>{if(request.status===401&&!location.pathname.startsWith('/login')){location.href='/login';return}try{resolve({ok:request.status>=200&&request.status<300,status:request.status,data:JSON.parse(request.responseText||'{}')})}catch(error){reject(error)}};request.onerror=()=>reject(new Error('Network request failed'));request.send(body?JSON.stringify(body):null)})}
function jsonp(kind){return new Promise((resolve,reject)=>{const callback=`dashboardData${Date.now()}${Math.floor(Math.random()*10000)}`,script=document.createElement('script'),timer=setTimeout(()=>cleanup(new Error('Data request timed out')),8000);const cleanup=error=>{clearTimeout(timer);delete window[callback];script.remove();error?reject(error):null};window[callback]=data=>{cleanup();if(data&&data.__unauthenticated__){location.href='/login';return}resolve(data)};script.onerror=()=>cleanup(new Error('Data request failed'));script.src=`/api/dashboard/${kind}?callback=${callback}`;document.head.append(script)})}
